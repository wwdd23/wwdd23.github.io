...
// (人数说明)  带括号内容为最终输出结果项
// 抓取乐天方案详情页，生成价格日历任务。
// 每个方案中每个房间，每个人数使用,生成六个月的价格日历任务
//
// 生成评论任务 任务URL
//  f_view_url=  'http://review.travel.rakuten.co.jp/hotel/api/' +house_id[0]+"?f_type=jsonp&f_plan=" +planId+ "&f_reviewNo=0&f_rows=1&callback=jQuery" + random_a +"&render=jsonp&_=" + random_b;
// 
// 试例:
// ./kancho_rakuten_hotelinfo.js 'http://hotel.travel.rakuten.co.jp/hotelinfo/plan/20212' /plan/ID 即可
// 生成任务后，执行脚本为: kancho_rakuten_hotel_calendar.js

var $ = require('cheerio');
var request = require('request');
var fs = require('fs');

var url = process.argv[2] ||'http://hotel.travel.rakuten.co.jp/hotelinfo/plan/20212?f_tscm_flg=&f_tel=&f_teikei=quick&f_heya_su=1&f_otona_su=1&f_s1=0&f_s2=0&f_y1=0&f_y2=0&f_y3=0&f_y4=0&f_kin=&f_kin2=&f_squeezes=&f_static=1';
...
request(url, function(error, response, body) {
  ...
  $(".planThumb").each(function(){  
    ...
    var planInfo = $(this).find('.planThumb div.htlPlnInfo p.htlPlnDtlPrv').text().trim(); //方案详情
    ...
    //生成评论任务
    var random_a = Math.floor(Math.random()*1000000);
    var random_b = Math.floor(Math.random()*1000000);
    var f_view_url=  'http://review.travel.rakuten.co.jp/hotel/api/' +house_id[0]+"?f_type=jsonp&f_plan=" +planId+ "&f_reviewNo=0&f_rows=1&callback=jQuery" + random_a +"&render=jsonp&_=" + random_b;
    task.push({
      "url" : f_view_url,
      "project" : 'rakuten',
      "category": 'normal',
      "script_name" : 'rakuten/rakuten_review_info.js',
      "context" : JSON.stringify({"planId" : planId, "hotelid" : house_id[0]})

    });


    //循环房型列表
    $(this).find('.htlPlnRmTypLst').each(function() {

      //var room_code = $(this).find('li').attr('id');
      $(this).find('.htlPlnRmTypInfo').each(function() {
        ...
        $(this).next().find('li').each(function(){
          ...
          for (var i =0 ; i < 6 ; i++){
            ...
            task.push({
              ...
              "project" : 'rakuten',
              "category" : 'normal',
              "script_name" : 'rakuten/rakuten_calendar_hotel.js',
              "context" : ""
      ...
      }); //-->  $(this).find('.htlPlnRmTypInfo').each(
    }); //--> $(this).find('.htlPlnRmTypLst').each(function()
  });  // --->   $(".planThumb").each(function()

  console.log(JSON.stringify({'status': 200, 'task':task,'result': result},undefined,3));
//  console.log(JSON.stringify({'status': 200, 'task':task,'result': result}));
